...
<section <?php if((get_field('section_id')) == true) { echo'id="' . get_field('section_id') . '"'; } else { ?>id="<?php echo $id; ?>"<?php } ?> class="grid-10 fl bg-white clearfix">
	<div class="grid-page mauto pad clearfix" style="<?php if(get_field('icon_perks_padding_top')) { echo 'padding-top:' . get_field('icon_perks_padding_top') . ';'; } ?> <?php if(get_field('icon_perks_padding_bottom')) { echo 'padding-bottom:' . get_field('icon_perks_padding_bottom') . ';'; } ?>">
		<?php if((get_field('icon_perks_headline') || get_field('icon_perks_subheadline') || get_field('icon_perks_text')) == true ) { echo '<div class="grid-10 fl pb5 clearfix">'; } ?>
			<?php if(get_field('icon_perks_headline')) { echo '<h2 class="block-headline bkw gr-text-pink-dark-purple '; } ?><?php if(get_field('icon_perks_headline_color')) { echo '' . get_field('icon_perks_headline_color') . ''; } ?><?php if(get_field('icon_perks_headline')) { echo '">' . get_field('icon_perks_headline') . '</h2>'; } ?>
				 ...
				 <?php if(get_sub_field('icon_perks_button_link')) { echo '<a href="' . get_sub_field(' icon_perks_button_link') . '" role="link">'; } ?>
					<span class="icon-content">
						<?php if(get_sub_field('icon_perks_icon')) { ?>
							...
							<img class="responsive-img animatefade fadeOut" src="<?php echo $image['url']; ?>" alt="<?php echo $image['alt']; ?>" />
		...
		@media screen and (max-width: 500px) {
			.icon-perks-list-flex li,
			.icon-perks-list-flex li.two,
			.icon-perks-list-flex li.three,
			.icon-perks-list-flex li.four, 
			.icon-perks-list-flex li.five{width:100%;max-width:100%;} 
		} */
		</style> 
	</div>   
